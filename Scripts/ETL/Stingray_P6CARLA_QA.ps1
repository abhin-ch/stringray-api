...
{	
	...
	#Populate ETL Log
	$sql = "UPDATE stng.ETLLog SET LUD = GETDATE(), LUB = '$machineid', ETLMethod = 'Powershell on BP Network Machine' WHERE ETLName = 'P6CARLA_QA'"
    $cmd.commandtext = $sql 
	$cmd.executenonquery()
    ...
    $sql = "SELECT P.PROJ_SHORT_NAME INTO #TempProjects
            ...
            SELECT W.WBS_ID
                ...
                ,W.PARENT_WBS_ID
                INTO #TempSubFragnet
            FROM [P6_DV].[dbo].[V_P6.P6_ProjWbs_Daily_Current] W 
            WHERE W.WBS_ID IN (SELECT DISTINCT A.WBS_ID
                FROM [P6_DV].[dbo].[P6_Task] A 
                --INNER JOIN [P6_DV].[dbo].[P6_TaskActv] B ON A.TASK_ID = B.TASK_ID AND B.ACTV_CODE_TYPE_ID = 427380 AND A.[CURRENT] = 'Y' AND B.[CURRENT] = 'Y'
                ...
                WHERE P.PROJ_SHORT_NAME IN (SELECT Proj.PROJ_SHORT_NAME FROM #TempProjects Proj)
            );
            ...
            SELECT W.WBS_ID
	            ...
	            ,NULL
            FROM [P6_DV].[dbo].[V_P6.P6_ProjWbs_Daily_Current] W 
            WHERE W.WBS_ID IN (SELECT S.PARENT_WBS_ID FROM #TempSubFragnet S)"
           

    $starttime = $stopwatch.elapsed.totalminutes
    & "$path" -fromservername "LSN_PSTG001,1572" -fromdbname "P6_DV" -destservername "LSN_BE_D,1569" -destdbname "ENG_STG" -destschemaname "stngQA" -desttblname "Fragnet" -destwinauthentication -fromwinauthentication -execsql $sql
	
    $sql = "ALTER TABLE [ENG_STG].[stngQA].[Fragnet] ALTER COLUMN FragnetID NUMERIC(10,0)
    ...
    $sql = "SELECT P.PROJ_SHORT_NAME INTO #TempProjects
            ...
            SELECT DISTINCT T.PROJ_ID
                ...
                ,T.TARGET_WORK_QTY
                ,RX.PLANNEDDURATION
                ,IIF(T.STATUS_CODE = 'Active',T.REEND_DATE,T.TARGET_END_DATE_D)
            FROM [P6_DV].[dbo].[P6_Task] T
            ...
            WHERE P.PROJ_SHORT_NAME IN (SELECT PP.PROJ_SHORT_NAME FROM #TempProjects PP)
            ORDER BY T.TASK_CODE ASC"

    $starttime = $stopwatch.elapsed.totalminutes
	& "$path" -fromservername "LSN_PSTG001,1572" -fromdbname "P6_DV" -destservername "LSN_BE_D,1569" -destdbname "ENG_STG" -destschemaname "stngQA" -desttblname "FragnetActivity" -destwinauthentication -fromwinauthentication -execsql $sql

	write-host "Creating indexes..."

    $sql = "ALTER TABLE [ENG_STG].[stngQA].[FragnetActivity] ALTER COLUMN ProjID NUMERIC(10,0)
            ...
            ALTER TABLE [ENG_STG].[stngQA].FragnetUpdate ALTER COLUMN RevisedCommitmentDate DATE;
            ...
            ALTER TABLE [ENG_STG].[stngQA].[FragnetActivity] ALTER COLUMN RemainingHours NUMERIC(10,0)
            ALTER TABLE [ENG_STG].[stngQA].[FragnetActivity] ALTER COLUMN BudgetedHours NUMERIC(10,0)
    ...
    $sql = "INSERT INTO stng.ActionLog(Action,ObjectName,StartTime,EndTime,ExecutionMessage)
            VALUES ('ETL','P6 Migration QA','$ETL_starttime',GETDATE(),'Success')"

    $cmd.commandtext = $sql 
	$cmd.executenonquery()

    #Run additional stored proc
	$cmd.commandtext = "EXEC [stngQA].[SP_0024_PostCARLAImport] @Operation=1"
	$cmd.executenonquery()
...
catch 
{
    ...
    $sql = "INSERT INTO stng.ActionLog(Action,ObjectName,StartTime,EndTime,ExecutionMessage)
            VALUES ('ETL','Stingray_P6CARLA.ps1','$ETL_starttime',GETDATE(),'Failed: $errorstr')"

    $cmd.commandtext = $sql 
	$cmd.executenonquery()
    $conn.close()

	[void](new-item -path "$psscriptroot\Errors" -name "$date-P6MigrationErrors.txt" -value $errorstr -itemtype "file")

...
{	
	...
	#Populate ETL Log
	$sql = "UPDATE stng.ETLLog SET LUD = GETDATE(), LUB = '$machineid', ETLMethod = 'Powershell on BP Network Machine' WHERE ETLName = 'P6Project_QA'"
    $cmd.commandtext = $sql 
	$cmd.executenonquery()
	...
	& "$path" -fromservername "LSN_PSTG001,1572" -fromdbname "P6_DV" -destservername "LSN_BE_D,1569" -destdbname "ENG_STG" -destschemaname "temp" -desttblname "FragnetActivity_QA" -destwinauthentication -fromwinauthentication -execsql $sql

    $CurrentTime = [DateTime]::Now | Get-Date -Format "yyyy-MM-dd HH:mm:ss"

    $sql = "UPDATE stngQA.FragnetActivity SET 
                [ActivityName] = A.ActivityName
                ,[StartDate] = A.StartDate
                ,[BLStartDate] = A.BLStartDate
                ,[EndDate] = A.EndDate
                ...
                ,[Status] = A.[Status]
            FROM temp.FragnetActivity_QA A
            WHERE stngQA.FragnetActivity.ActivityID = A.ActivityID;
            
            UPDATE stngQA.MPL SET LastUpdated = '$CurrentTime'
            FROM temp.FragnetActivity_QA A 
            WHERE stngQA.MPL.[Project ID] = A.ProjShortName
            
            DROP TABLE temp.FragnetActivity_QA"
...
catch 
{
	...
	switch (get-childitem -path "$psscriptroot\Errors") 
	{
		...
		{$_.name -match "$date-P6MigrationProjectErrors"} {
    ...
    $sql = "UPDATE stngQA.MPL SET LastUpdated = 'Error'
            WHERE stngQA.MPL.[Project ID] IN ($($args[0]));

CREATE OR ALTER VIEW [stng].[VV_MPL_AllProjectData]
AS

with mpl as(
select * ,case when DEPARTMENTSHORT='ICE' then 'EIC'
				when DEPARTMENTSHORT='EP' then 'PC'
				when DEPARTMENTSHORT='MCD' then 'MEC' else DEPARTMENTSHORT end as DEPARTMENTSHORT2

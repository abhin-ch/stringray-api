...
(
	...
	INNER JOIN p6hours AS y 
		ON y.SDQUID = x.SDQUID 
		AND y.PersonGroup = x.PersonGroup
	...
	SELECT 
		...
		x.Comment
	FROM sms AS x
	INNER JOIN stng.VV_Budgeting_SDQMain AS C 
		ON C.SDQApprovalSetID = x.SDQApprovalSetID 
		AND C.Section = x.PersonGroup
	WHERE NOT EXISTS (
		SELECT 1
		FROM p6hours AS y
		WHERE y.SDQUID = x.SDQUID 
			AND y.PersonGroup = x.PersonGroup
	)
)
SELECT DISTINCT 

CREATE OR ALTER FUNCTION [stng].[FN_Budgeting_SDQ_EngineeringEAC]
(
    @SDQUID BIGINT
)
RETURNS @Tbl TABLE
(
    CDPhaseName VARCHAR(50),
    PGType VARCHAR(50),
    Cost FLOAT
)
AS
BEGIN
    ...
    BEGIN
        INSERT INTO @Tbl (CDPhaseName, PGType, Cost)
        SELECT CDPhaseName, PGType, Cost
        FROM temp.SDQ_P6_Legacy_PDFChartData
        WHERE SDID = @SDQUID
    END
    ELSE
    BEGIN
        ...
        secondquery AS (
            ...
            INNER JOIN stngetl.VV_Budgeting_P6_ActivityResource AS b 

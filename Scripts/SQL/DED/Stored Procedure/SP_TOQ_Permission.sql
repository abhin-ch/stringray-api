CREATE OR ALTER PROCEDURE [stng].[SP_TOQ_Permission]
(
	 @Operation TINYINT
	,@SubOp		TINYINT = NULL
...
BEGIN
	...
	ELSE IF(@Operation = 2)
	BEGIN
		SELECT B.BPRole, B.EmployeeID, B.Vendor
		...
		FROM (
			...
			LEFT JOIN stng.[VV_Admin_UserAttribute] A ON A.EmployeeID = U.EmployeeID 
				AND (A.AttributeType = 'BPRole' OR A.AttributeType = 'Vendor')
			WHERE U.EmployeeID = @EmployeeID
			OR U.EmployeeID IN (
				SELECT [Delegator]
				FROM [stng].[VV_Admin_Delegation]
				WHERE Active = 1 
				AND Status = 'Active'
				AND Delegatee = @EmployeeID
			)
		) B

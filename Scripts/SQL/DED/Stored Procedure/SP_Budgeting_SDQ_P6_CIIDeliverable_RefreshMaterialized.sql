CREATE OR ALTER   PROCEDURE [stngetl].[SP_Budgeting_SDQ_P6_CIIDeliverable_RefreshMaterialized]
(
...
BEGIN
    ...
    ------------------------------------------------------------
    DELETE FROM stngetl.Budgeting_SDQ_P6_CIIDeliverable_Materialized
    WHERE SDQUID = @SDQUID;

    ------------------------------------------------------------
    -- 2. Insert rows into P6_CIIDeliverable_Materialized
    ------------------------------------------------------------
    WITH lvl6wbs AS (
        SELECT DISTINCT 
            b.SDQUID, a.RunID, a.wbs_name, a.wbs_code, a.activityid,
            ...
            c.SavingPCT,
            c.CommittedSaving
        FROM stng.Budgeting_SDQP6Link b
        JOIN stngetl.Budgeting_SDQ_Run f ON b.RunID = f.RunID AND f.Legacy = 0
        JOIN stngetl.Budgeting_SDQ_P6 a ON a.RunID = b.RunID AND a.CV = 0 AND a.wbs_lvl = 6
        JOIN stng.Budgeting_SDQ_StandardDeliverable c ON a.DeliverableType = c.DeliverableID AND c.Deleted = 0
        LEFT JOIN stngetl.VV_Budgeting_P6_ActivityResource d ON a.task_id = d.task_id AND a.RunID = d.runid AND d.Baseline = 0
        WHERE b.Active = 1 AND a.RunID = @RunID
    ),
    lvl6wbssum AS (
        SELECT 
            SDQUID, RunID, wbs_name, wbs_code,
            DeliverableType, Unit,
            DeliverableName, Direct, PhaseCode, DisciplineCode,
            MIN(CurrentStart) AS CurrentStart,
            MAX(CAST(StartActualized AS TINYINT)) AS StartActualized,
            MAX(CurrentEnd) AS CurrentEnd,
            MIN(CAST(EndActualized AS TINYINT)) AS EndActualized,
    ...
    childwbs AS (
        SELECT DISTINCT 
            b.SDQUID, a.RunID, a.wbslvl6, a.activityid,
            COALESCE(d.LabourActualUnits,0) AS LabourActualUnits,
            COALESCE(d.LabourActualCost,0) AS LabourActualCost,
            COALESCE(d.NonLabourActualUnits,0) AS NonLabourActualUnits,
            COALESCE(d.NonLabourActualCost,0) AS NonLabourActualCost,
    ...
    initialquery AS (
        SELECT 
            a.SDQUID, a.RunID, a.wbs_name, a.wbs_code,
            a.DeliverableType, a.DeliverableName, a.unit, a.direct,
            a.phasecode, a.disciplinecode,

            MIN(
                CASE 
                    WHEN a.CurrentStart < b.CurrentStart OR b.CurrentStart IS NULL 
                    THEN a.CurrentStart ELSE b.CurrentStart 
                END
            ) AS CurrentStart,
            ...
            CAST(
                MAX(
                    CASE 
                        WHEN CAST(a.StartActualized AS TINYINT) > CAST(b.StartActualized AS TINYINT)
                             OR b.StartActualized IS NULL
                        THEN CAST(a.StartActualized AS TINYINT)
                        ELSE CAST(b.StartActualized AS TINYINT)
                    END
                ) AS BIT
            ) AS StartActualized,
            ...
            MAX(
                CASE 
                    WHEN a.CurrentEnd > b.CurrentEnd OR b.CurrentEnd IS NULL
                    THEN a.CurrentEnd ELSE b.CurrentEnd 
                END
            ) AS CurrentEnd,
            ...
            CAST(
                MIN(
                    CASE 
                        WHEN CAST(a.EndActualized AS TINYINT) < CAST(b.EndActualized AS TINYINT)
                             OR b.EndActualized IS NULL
                        THEN CAST(a.EndActualized AS TINYINT)
                        ELSE CAST(b.EndActualized AS TINYINT)
                    END
                ) AS BIT
            ) AS EndActualized,
        ...
        FROM lvl6wbssum a
        LEFT JOIN childwbssum b ON a.RunID = b.RunID AND a.wbs_code = b.wbslvl6
    ...
    ------------------------------------------------------------
    -- 3. Insert calculated rows
    ------------------------------------------------------------
    INSERT INTO stngetl.Budgeting_SDQ_P6_CIIDeliverable_Materialized (
        UniqueID,
        ...
        BIMSEstimate, BIMSCommit, FinalBIMSCommit,
        Legacy
    )
    SELECT 
        NEWID(),

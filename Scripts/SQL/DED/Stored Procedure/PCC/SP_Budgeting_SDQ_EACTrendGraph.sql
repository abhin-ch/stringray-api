CREATE OR ALTER PROCEDURE [stng].[SP_Budgeting_SDQ_EACTrendGraph] 
(
    @SDQUID bigint
)
AS
BEGIN
    ...
    -- Get ProjectNo and LAMP4 for the requested SDQUID
    WITH ProjectData AS (
        SELECT A.ProjectNo, A.LAMP4, A.RecordTypeUniqueID AS SDQUID
        FROM stng.VV_Budgeting_SDQMain A
        WHERE A.RecordTypeUniqueID = @SDQUID
    ),
    Lamp4 AS (
        SELECT P.SDQUID, E.EAC
    ...
    SELECT * INTO #Lamp4 FROM Lamp4;

    -- Previous Revision branch
    SELECT 
    ...
    -- Final union
    SELECT SDQUID, Phase, Revision, [Label], LAMP3, EcoSysEAC, EAC, LAMP4, EcoSysEAC_ENG
    FROM #Prev
    UNION ALL
    SELECT SDQUID, Phase, Revision, [Label], LAMP3, EcoSysEAC, EAC, LAMP4, EcoSysEAC_ENG
    FROM #Curr;
